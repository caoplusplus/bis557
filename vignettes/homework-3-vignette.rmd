...
lasso_reg_with_screening <- function(X, y, lambda) {
  fit <- glmnet::glmnet(X, y, alpha = 1, lambda = lambda)
...
#data(Boston)
#sample <- sample(1:nrow(Boston), size = 100, replace = FALSE)
#train_B <- Boston[-sample, ]
#xtrain <- as.matrix(train_B[, -14])
#ytrain <- train_B$medv
#lasso_reg_with_screening(xtrain, ytrain, 0.3)
```
